...
###Install packages if needed, set working directory 
```
...
###The scripts below take in DESeq output comparisons (merged with bind_rows) with an additional column saying which comparison.  A simple pipeline to do this is below
```
...
#### Example: Flow 1 shows gene behavior over a timecourse (damage vs. undamaged at 1hr and 6hr), and flow 2 shows how the same genes behave after inceptin treatment
#### Three conditions is more complicated to visualize; by default the two spacer sections between axes have no relation to one another (so a given gene's position does not "flow" from condition 1->2->3).  Instead we have to specify ggflow(stat="alluvium").  This takes longer to render and looks more complicated, but correctly shows gene flows across the entire diagram
#### Because more flows complicates the diagrams, it's best to only have 3 classes.  Below, instead of 2 tiers of higher/lower there is only 1 tier
...
set <- deseq_input %>% 
  ...
  #Mutates a new column "class" that classifies genes that meet significance threshold and log2FoldChange between the high/low variables specified above
  mutate(class = ifelse(is.na(sig),otherwise,
                    ifelse(sig==1,
                      ifelse(log2FoldChange>higher,higher,
                      ifelse(log2FoldChange>high,high,
...
### dpyrl is also great for post-filter analysis; e.g. the following example produces a list of 235 genes that go from upregulated (in comparison 1) to no differential expression (comp 2) to upregulated (comp 3)
```{r}
list <- filter(set,
...
# Old code is included in the R markdown file below but hidden in the final html file.  It might contain some useful snippets
